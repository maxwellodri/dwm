...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, ispermanent, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow, attachdirection;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;
...
applyrules(Client *c)
{
	...
	for (i = 0; i < LENGTH(rules); i++) {
		...
		&& (!r->instance || strstr(instance, r->instance)))
		{
			...
			c->tags |= r->tags;
            c->attachdirection = r->attachdirection;
...
drawbar(Monitor *m)
{
	...
	x = drw_text(drw, x, 0, blw, bh, lrpad / 2, m->ltsymbol, 0);
...
{
    Client *prev = getclientundermouse();
